...
class UserIDPredictor:
    ...
    def load_and_preprocess_data(self, file_path=None, df=None):
        ...
        print(f"Users: {self.df['Username'].nunique()}")
        print("\nUser distribution:")
        print(self.df['Username'].value_counts())
        
        # Feature engineering
        self.df = self._engineer_features(self.df)
        
        # Prepare features and target
        feature_columns = [col for col in self.df.columns if col not in ['Username', 'problem_id']]
    ...
    def analyze_features(self, X, y):
        ...
        df_with_user = X.copy()
        df_with_user['Username'] = self.label_encoder.inverse_transform(y)
        
        print("\nFeature Means by User:")
        user_stats = df_with_user.groupby('Username').agg({
            'cpu_time_min': 'mean',
            'memory_mean': 'mean',
            'code_size_mean': 'mean',
            'code_density': 'mean'
